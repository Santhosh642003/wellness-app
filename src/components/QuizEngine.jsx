      ...
      <div className="min-h-screen" style={{ background: "var(--bg-page)" }}>
        <div className="max-w-4xl mx-auto px-6 py-10">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-600 dark:text-gray-400">{title}</p>
            <div className="mt-4 inline-flex items-center gap-6 bg-slate-50 dark:bg-[#141414] border border-slate-200 dark:border-gray-800 rounded-2xl px-8 py-4">
              <div className="text-center">
                ...
                <div className="text-xs text-slate-600 dark:text-gray-500 mt-0.5">Score</div>
              </div>
              ...
              <div className="text-center">
                ...
                <div className="text-xs text-slate-600 dark:text-gray-500 mt-0.5">Correct</div>
              </div>
              <div className="h-8 w-px bg-slate-200 dark:bg-gray-700" />
              <div className="text-center">
                <div className="text-3xl font-bold text-blue-400">{score}</div>
                <div className="text-xs text-slate-600 dark:text-gray-500 mt-0.5">Points</div>
              </div>
            </div>
          </div>
              ...
              return (
                <div key={i} className={`bg-slate-50 dark:bg-[#141414] border rounded-2xl p-5 ${isCorrect ? "border-emerald-400/40" : unanswered ? "border-gray-600/40" : "border-red-400/40"}`}>
                  <div className="flex items-start justify-between gap-4 mb-3">
                    ...
                    <span className={`shrink-0 text-xs font-semibold px-2 py-0.5 rounded-full ${isCorrect ? "bg-emerald-400/15 text-emerald-400" : unanswered ? "bg-gray-400/15 text-gray-600" : "bg-red-400/15 text-red-400"}`}>
                  ...
                  {q.explanation && (
                    <p className="text-slate-600 dark:text-gray-500 text-xs mt-3 italic">{q.explanation}</p>
                  )}
                </div>
              );
            })}
    ...
    <div className="min-h-screen" style={{ background: "var(--bg-page)" }}>
      <div className="max-w-6xl mx-auto px-6 py-10">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-600 dark:text-gray-400 mt-2">{subtitle}</p>
        </div>

        {/* Progress bar */}
        <div className="mb-6">
          <div className="flex justify-between text-xs text-slate-600 dark:text-gray-500 mb-2">
        ...
        <div className="bg-slate-50 dark:bg-[#141414] border border-slate-200 dark:border-gray-800 rounded-2xl p-8 shadow-sm">
          <div className="flex items-center justify-between text-sm text-slate-600 dark:text-gray-400 mb-6">
          ...
          <div className="mt-8 flex items-center justify-between">
            <button
              ...
              className="px-5 py-3 rounded-xl text-sm font-medium border border-slate-200 dark:border-gray-800 text-slate-600 dark:text-gray-400 hover:text-slate-700 dark:hover:text-gray-200 disabled:opacity-30 disabled:cursor-not-allowed"
            >
              ← Previous
            </button>
            <button
              onClick={goNext}
              className={`px-6 py-3 rounded-xl font-semibold transition ${answers[currentIndex] !== null ? "bg-gradient-to-r from-blue-500 to-emerald-400 text-white hover:opacity-90" : "bg-slate-100 dark:bg-[#1a1a1a] text-slate-600 dark:text-gray-500"}`}
            >
          ...
          <div className="mt-6 flex flex-wrap gap-2 justify-center">
            {questions.map((_, i) => (
              <button
                ...
                className={`h-7 w-7 rounded-full text-xs font-medium border transition ${i === currentIndex ? "border-blue-400 bg-blue-500/20 text-blue-400" : answers[i] !== null ? "border-emerald-400 bg-emerald-400/10 text-emerald-400" : "border-slate-200 dark:border-gray-700 text-slate-600 dark:text-gray-500"}`}
              >
                {i + 1}
              </button>
            ))}
